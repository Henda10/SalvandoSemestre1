            ...
            <div class="flex items-center mr-5 my-3">
                ...
                <div class="text-center">
                    <button
                        ...
                        <div>
                            <a href="{{route('motociclistas.create')}}" class="m-1">Nuevo motociclista</a>
                        </div>
                    </button>
                </div>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 my-3">
                <thead class="bg-gray-100 text-center border divide-y">
                    <tr>
                        ...
                        <th class="border px-2 py-2">Moto asignada</th>
                        <th class="border px-2 py-2">Acciones</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y min-w-full divide-gray-200">
                    @foreach ($motocicleGuy as $guy)
                        
                    <tr class="text-center">
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->id}}</td>
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->Primernombre}}</td>
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->Segundonombre}}</td>
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->PrimerApellido}}</td>
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->SegundoApellido}}</td>
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->Numero}}</td>
                        <td class="border px-2 py-2 whitespace-nowrap">{{$guy->fk_motos}}</td>
                        {{-- Acciones --}}
                        <td class="border px-2 py-2 flex justify-around items-center">
                            {{-- editar --}}
                            <a href="{{route('motociclistas.edit', $guy->id)}}">
                                <button
                            ...
                            {{-- eliminar --}}
                            <form action="{{route('motociclistas.destroy',$motocicleGuy)}}"method="POST">
                                @csrf
                                @method('DELETE')
                                <button
